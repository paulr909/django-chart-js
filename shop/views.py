...
def get_filter_options(request):
    grouped_purchases = (
        Purchase.objects.annotate(year=ExtractYear("time"))
        .values("year")
        .order_by("-year")
        .distinct()
    )
...
def get_sales_chart(request, year):
    ...
    grouped_purchases = (
        purchases.annotate(price=F("item__price"))
        .annotate(month=ExtractMonth("time"))
        .values("month")
        .annotate(average=Sum("item__price"))
...
def spend_per_customer_chart(request, year):
    ...
    grouped_purchases = (
        ...
        .annotate(month=ExtractMonth("time"))
        .values("month")
        .annotate(average=Avg("item__price"))
        .values("month", "average")
        .order_by("month")
    )
...
def payment_method_chart(request, year):
    ...
    for group in grouped_purchases:
        payment_method_dict[
            dict(Purchase.PAYMENT_METHODS)[group["payment_method"]]
        ] = group["count"]
    ...
    return JsonResponse(
        {
            ...
            "data": {
                ...
                "datasets": [
                    {
                        "label": "Amount (£)",
                        "backgroundColor": generate_color_palette(
                            len(payment_method_dict)
                        ),
